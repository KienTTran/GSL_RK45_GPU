...
//        }
        ...
        for(int i = 0; i < NUMSEROTYPES; i++){
            ...
            flu_params_new_d->G_CLO_BETA[beta_index] = flu_params_current_d->G_CLO_BETA[beta_index] + flu_params_current_d->beta_sd[i]*curand_normal(&local_state);
            flu_params_new_d->beta[beta_index] = flu_params_new_d->G_CLO_BETA[beta_index] * BETA_OVER_POP_MAIN;
        }
        /* 3 */
        ...
        for(int i = 1; i < SAMPLE_PHI_LENGTH; i++){
            ...
            flu_params_new_d->tau[tau_index] = flu_params_current_d->tau[tau_index] + flu_params_current_d->tau_sd[(i - 1)]*curand_normal(&local_state);
