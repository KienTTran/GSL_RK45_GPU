...
__device__
double seasonal_transmission_factor(GPUParameters *gpu_params, double t) {
    /*
...
void rk45_gpu_evolve_apply(double t, double t_target, double t_delta, double h, double *y[], double *y_output[],
                           double *y_output_agg[], int index, GPUParameters *params) {
    double device_y[DIM];
...
//            y_output[index][y_output_index] = pop_sum(device_y);
        ...
        if ((day+1) % 7 == 0 || day == NUMDAYSOUTPUT - 1) {
            for(int i = 0; i < params->data_params.cols; i++){
                //Col 3 4 5
...
__device__
void
solve_ode(double *y_ode_input_d[], double *y_ode_output_d[], double *y_ode_agg_d[], int index, GPUParameters *params) {
...
__global__
void solve_ode(double *y_ode_input_d[], double *y_ode_output_d[], double *y_ode_agg_d[], GPUParameters *params) {
