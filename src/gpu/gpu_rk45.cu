...
void rk45_gpu_evolve_apply(double t, double t_target, double t_delta, double h, double *y[], double *y_output[],
                           double *y_output_agg[], int index, GPU_Parameters *params) {
    double device_y[DIM];
...
//        week = __float2int_ru(day/7);
//        for(int i = 0; i < params->data_params.cols; i++){
//            const int y_output_agg_col = (3 + i) + week * params->agg_dimension;
...
__device__
void
solve_ode(double *y_ode_input_d[], double *y_ode_output_d[], double *y_ode_agg_d[], int index, GPU_Parameters *params) {
    rk45_gpu_evolve_apply(params->t0, params->t_target, params->step, params->h, y_ode_input_d, y_ode_output_d,
                          y_ode_agg_d, index, params);
    return;
}
...
void mcmc_dnorm(double *y_data_input_d[], double *y_ode_agg_d[], double* y_mcmc_dnorm_d[], GPU_Parameters *params) {
    int index_gpu = threadIdx.x + blockIdx.x * blockDim.x;
    int stride = blockDim.x * gridDim.x;
...
__global__ /* Each thread will calculate 1 line in y_ode_agg_d */
void mcmc_get_dnorm_outputs_2d(double* d_mcmc_dnorm_d[], double* d_mcmc_dnorm_h1_d[],double* d_mcmc_dnorm_b_d[],double* d_mcmc_dnorm_h3_d[], GPU_Parameters* params){
    int index_gpu = threadIdx.x + blockIdx.x * blockDim.x;
    int stride = blockDim.x * gridDim.x;
...
__global__ /* Each thread will calculate 1 line in y_ode_agg_d */
void mcmc_get_dnorm_outputs_1d(double* d_mcmc_dnorm_h1_b_h3_d[], double d_mcmc_dnorm_h1_1d_d[],double d_mcmc_dnorm_b_1d_d[],double d_mcmc_dnorm_h3_1d_d[], GPU_Parameters* params){
    int index_gpu = threadIdx.x + blockIdx.x * blockDim.x;
    int stride = blockDim.x * gridDim.x;
...
//        mcmc_get_dnorm_outputs_2d<<<params->num_blocks, params->block_size>>>(y_mcmc_dnorm_h1b_h3_d, y_mcmc_dnorm_h1_d, y_mcmc_dnorm_b_d, y_mcmc_dnorm_h3_d, params_d);
        ...
        checkCuda(cudaMemcpy(dnorm_sum_h,y_mcmc_dnorm_h3_1d_d,params->data_params.rows * sizeof(double), cudaMemcpyDeviceToHost));
        double sum = 0.0;
        for(int i = 0; i < params->data_params.rows; i++){
            sum += dnorm_sum_h[i];
        }
        printf("sum y_mcmc_dnorm_h1_1d_d = %.5f\n",sum);
