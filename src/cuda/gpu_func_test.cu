...
//    printf("[function] trr = %.10f\n",trr);
            ...
            for(stg=0; stg<NUMR; stg++)
            {
                ...
                for(int l=0; l<NUMLOC; l++)
                    for(int v=0; v<NUMSEROTYPES; v++)
                        sum_foi += gpu_params->sigma[vir][v] * gpu_params->beta[v] * gpu_params->eta[loc][l] * y[ STARTI + NUMSEROTYPES*l + v ];
    ...
    for(loc=0; loc<NUMLOC; loc++)
    {
        for(vir=0; vir<NUMSEROTYPES; vir++)
        {
            ...
            for(int l=0; l<NUMLOC; l++)
                foi_on_susc_single_virus += gpu_params->eta[loc][l] * gpu_params->beta[vir] * y[ STARTI + NUMSEROTYPES*l + vir ];
    ...
    for(loc=0; loc<NUMLOC; loc++)
    {
        ...
        for(int l=0; l<NUMLOC; l++)
            for(int v=0; v<NUMSEROTYPES; v++)
                foi_on_susc_all_viruses += gpu_params->eta[loc][l] * gpu_params->beta[v] * y[ STARTI + NUMSEROTYPES*l + v ];
